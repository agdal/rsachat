...
const vm = new Vue ({
  ...
  async created () {
    this.addNotification('Velkommen! Opretter et nyt keypair...')
    ...
    this.addNotification(`Keypair oprettet: ${this.getKeySnippet(this.originPublicKey)}`)
  ...
  methods: {
    ...
    setupSocketListeners () {
      ...
      this.socket.on('connect', () => {
        this.addNotification('Forbundet til serveren')
        this.joinRoom()
      })

      // Notify user that they have lost the socket connection
      this.socket.on('disconnect', () => this.addNotification('Forbindelse mistet'))
      ...
      this.socket.on('NEW_CONNECTION', () => {
        this.addNotification('En bruger har tilsluttet sig rummet!')
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_JOINED', (newRoom) => {
        ...
        this.addNotification(`Forbindelse oprettet til: ${this.currentRoom}`)
        this.sendPublicKey()
      })

      // Save public key when received
      this.socket.on('PUBLIC_KEY', (key) => {
        this.addNotification(`Public Key Modtaget: ${this.getKeySnippet(key)}`)
        this.destinationPublicKey = key
      })

      // Clear destination public key if other user leaves room
      this.socket.on('user disconnected', () => {
        this.notify(`Bruger forlod rummet: ${this.getKeySnippet(this.destinationKey)}`)
        this.destinationPublicKey = null
      })

      // Notify user that the room they are attempting to join is full
      this.socket.on('ROOM_FULL', () => {
        this.addNotification(`Kan ikke tilslutte ${this.pendingRoom}, rummet er fuld`)
    ...
    joinRoom () {
      if (this.pendingRoom !== this.currentRoom && this.originPublicKey) {
        this.addNotification(`Tilslutter til rummet: - ${this.pendingRoom}`)
